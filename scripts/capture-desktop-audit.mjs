...
if (!page) throw new Error("No browser page is available for the desktop audit.");
...
const capture = async name => {
  ...
  const path = `/home/ubuntu/desktop-audit-${name}.png`;
...
if (await evaluate(`document.body.innerText.includes('Welcome to') && document.body.innerText.includes('Take the 60-sec tour')`)) {
  ...
  await evaluate(`document.querySelector('.tour-card .close')?.click(); true`);
  await sleep(100);
  await clickText("Start planning");
  await sleep(130);
}
...
await sleep(100);
await evaluate(`(() => { const day = document.querySelector('.month-cell.today') || document.querySelector('.month-cell:not(.blank)'); if (!day) throw new Error('Calendar day missing'); day.dispatchEvent(new MouseEvent('contextmenu', { bubbles: true, cancelable: true, clientX: 420, clientY: 500 })); return true; })()`);
await sleep(120);
output.push(await capture("context-menu"));
