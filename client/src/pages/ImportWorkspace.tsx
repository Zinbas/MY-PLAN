...
export type ImportCandidate = { id: string; title: string; kind: ComposerKind; date: string; time: string; durationMinutes: number; course: string; notes: string; weekdays?: number[]; repeatUntil?: string; confidence: number; approved: boolean };
...
const formatTime12Hour = (value: string) => {
  ...
  const hours = Number(match[1]); const minutes = match[2];
...
export default function ImportWorkspace({ isAuthenticated, isScanning, inputRef, fileName, message, candidates, onChooseFile, onAddApproved, onSetAllApproved, canUndoLastImport, onUndoLastImport, onUpdateCandidate, onDiscard }: ImportWorkspaceProps) {
  ...
  const applyRange = () => {
    if (!selectedWeekly.length) { setSubmitError("Select at least one weekly timetable item before applying a date range."); return; }
    if (!weeklyStart || !weeklyEnd) { setSubmitError("Set both Starts on and Repeats until dates before applying them."); return; }
    if (weeklyEnd < weeklyStart) { setSubmitError("Repeats until must be on or after Starts on."); return; }
    selectedWeekly.forEach(candidate => onUpdateCandidate(candidate.id, { date: weeklyStart, repeatUntil: weeklyEnd }));
    setRangeApplied(true); setSubmitError("");
  };
  const confirm = () => {
