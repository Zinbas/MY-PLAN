...
export default function Home() {
  ...
  const mobileSheetDragStart = useRef<number | null>(null);
  ...
  const closeMobileDateAction = () => {
    setMobileDateAction(null);
    if (window.history.state?.myPlanMobileDateSheet) window.history.back();
  };
  const openMobileDateAction = (date: Date) => {
    setMobileDateAction(date);
    if (!window.history.state?.myPlanMobileDateSheet) window.history.pushState({ ...(window.history.state ?? {}), myPlanMobileDateSheet: true }, "", window.location.href);
  };
  useEffect(() => {
    const closeOnHistoryBack = () => setMobileDateAction(null);
  ...
  const openComposerForDate = (kind: ComposerKind, date: Date, enableReminder = false) => { closePopovers(); setDateContextMenu(null); closeMobileDateAction(); setSelectedDate(date); setCursor(monthStart(date)); setSection("calendar"); resetDraft(kind, date, enableReminder); setShowComposer(true); };
  ...
  return <div className={`ongoing-shell ${sidebarCollapsed ? "is-sidebar-collapsed" : ""} ${activeTimer ? "has-active-timer" : ""}`}>
    ...
    {section !== "welcome" ? <MobilePlannerNav activeSection={section} onOpen={openSection} onPlan={() => openMobileDateAction(selectedDate)} /> : null}
    ...
    {mobileDateAction ? <div className="mobile-date-sheet-backdrop" onPointerDown={event => { if (event.target === event.currentTarget) closeMobileDateAction(); }}><section className="mobile-date-sheet" role="dialog" aria-modal="true" aria-label={`Plan ${formatShort(mobileDateAction)}`} onPointerDown={event => { event.stopPropagation(); mobileSheetDragStart.current = event.clientY; }} onPointerUp={event => { const start = mobileSheetDragStart.current; mobileSheetDragStart.current = null; if (start !== null && event.clientY - start > 72) closeMobileDateAction(); }} onPointerCancel={() => { mobileSheetDragStart.current = null; }}><div className="mobile-date-sheet-heading"><div className="sheet-handle" /><button className="mobile-date-sheet-close" onClick={closeMobileDateAction} aria-label="Close planning actions"><X size={18} /></button></div><p className="kicker"><CalendarDays size={14} /> {formatShort(mobileDateAction)}</p><h2>What belongs here?</h2><p>Choose the kind of plan you want to place on this date.</p><button className="mobile-date-sheet-action" onClick={() => openComposerForDate("task", mobileDateAction)}><ListTodo size={17} /><span><strong>Add task</strong><small>Capture the next action</small></span><ArrowRight size={15} /></button><button className="mobile-date-sheet-action" onClick={() => openComposerForDate("event", mobileDateAction)}><CalendarPlus size={17} /><span><strong>Add event</strong><small>Protect time for it</small></span><ArrowRight size={15} /></button><button className="mobile-date-sheet-action" onClick={() => openComposerForDate("block", mobileDateAction)}><Clock3 size={17} /><span><strong>Add focus block</strong><small>Make room to concentrate</small></span><ArrowRight size={15} /></button><button className="mobile-date-sheet-action" onClick={() => openComposerForDate("task", mobileDateAction, true)}><BellRing size={17} /><span><strong>Set reminder</strong><small>Choose timing for this day</small></span><ArrowRight size={15} /></button></section></div> : null}
