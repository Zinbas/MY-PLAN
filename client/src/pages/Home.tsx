...
import { ArrowRight, BarChart3, Bell, BellRing, BookOpenCheck, CalendarDays, CalendarPlus, Check, ChevronLeft, ChevronRight, CirclePlus, Clock3, CloudCog, Copy, Edit3, ExternalLink, Flag, GraduationCap, ListChecks, ListTodo, LogIn, LogOut, Menu, MoreHorizontal, PanelLeftClose, PanelLeftOpen, Pause, Play, Plus, RefreshCw, Search, ShieldCheck, Settings2, Sparkles, Square, Star, Trash2, Upload, UserRound, Users, X } from "lucide-react";
...
export default function Home() {
  ...
  return <div className={`ongoing-shell ${sidebarCollapsed ? "is-sidebar-collapsed" : ""} ${activeTimer ? "has-active-timer" : ""}`}>
    ...
    <main className="ongoing-main" onPointerDownCapture={event => { const target = event.target as HTMLElement; if (!(target.closest(".control-anchor") || target.closest(".date-context-menu") || target.closest(".mobile-date-sheet"))) { closePopovers(); setDateContextMenu(null); setMobileDateAction(null); } }}><header className="ongoing-topbar"><button className="desktop-sidebar-toggle" onClick={() => setSidebarCollapsed(value => !value)} aria-label={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"} title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}>{sidebarCollapsed ? <PanelLeftOpen size={18} /> : <PanelLeftClose size={18} />}</button><button className="mobile-menu" onClick={() => setShowSidebar(value => !value)} aria-label={showSidebar ? "Close navigation" : "Open navigation"} title={showSidebar ? "Close navigation" : "Open navigation"}>{showSidebar ? <X size={19} /> : <Menu size={19} />}</button><span className="toast-line">{toast}</span><div className="top-actions"><button onClick={goToday}><CalendarDays size={15} /> Today</button><button onClick={() => openComposer("task")}><ListTodo size={15} /> Add task</button><button onClick={() => openComposer("event")}><CalendarPlus size={15} /> Add event</button><button onClick={() => openComposer("task", true)}><BellRing size={15} /> Reminder</button><button className="accent" onClick={() => openComposer("block")}><Plus size={16} /> Add block</button></div></header><nav className="quick-action-strip" aria-label="Quick actions"><button onClick={() => openComposer("task")}><ListTodo size={15} /><span><strong>Add task</strong><small>Next action</small></span></button><button onClick={() => openComposer("event")}><CalendarPlus size={15} /><span><strong>Add event</strong><small>Protect time</small></span></button><button onClick={() => openComposer("block")}><Clock3 size={15} /><span><strong>Focus block</strong><small>Make room</small></span></button><button onClick={() => openComposer("task", true)}><BellRing size={15} /><span><strong>Set reminder</strong><small>Choose timing</small></span></button></nav>
