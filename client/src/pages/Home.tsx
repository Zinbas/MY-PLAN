...
import type { PlannerSection } from "@/lib/plannerNavigation";
import { mobileSurfaceFor, type MobilePlannerSurface } from "@/lib/mobilePlannerNavigation";
...
import MobilePlannerNav from "./MobilePlannerNav";
...
export default function Home() {
  ...
  const [mobileSurface, setMobileSurface] = useState<MobilePlannerSurface>("today");
  ...
  const openSection = (next: Section) => { closePopovers(); setSection(next); setMobileSurface(mobileSurfaceFor(next)); setShowSidebar(false); };
  const openMobileSurface = (surface: MobilePlannerSurface) => {
    if (surface === "todo") openSection("todo");
    else if (surface === "more") openSection("tools");
    else { openSection("calendar"); setView(surface === "calendar" ? "month" : "agenda"); }
  ...
  const resetDraft = (kind: ComposerKind, date = selectedDate, enableReminder = false) => { setComposerKind(kind); setEditingId(null); setDraftTitle(""); setDraftDate(dateKey(date)); setDraftTime(kind === "block" ? "19:00" : "09:00"); setDraftDuration("60"); setDraftRepeat("none"); setDraftPriority("normal"); setDraftCourse(""); setDraftNotes(""); setDraftScheduleTask(true); setDraftReminderLeadMinutes(enableReminder ? 10 : ""); };
  ...
  const saveComposer = () => {
    ...
    if (composerKind === "event") {
      const next: PersonalEvent = { id: editingId ?? `event-${Date.now()}`, title: draftTitle.trim(), startAt, endAt: new Date(startAt.getTime() + duration * 60000), priority: draftPriority, course: draftCourse.trim() || "Personal", notes: draftNotes.trim(), reminderLeadMinutes: draftReminderLeadMinutes === "" ? undefined : draftReminderLeadMinutes };
  ...
  return <div className={`ongoing-shell ${sidebarCollapsed ? "is-sidebar-collapsed" : ""} ${activeTimer ? "has-active-timer" : ""}`}>
    ...
    <main className="ongoing-main" onPointerDownCapture={event => { const target = event.target as HTMLElement; if (!(target.closest(".control-anchor") || target.closest(".date-context-menu") || target.closest(".mobile-date-sheet"))) { closePopovers(); setDateContextMenu(null); setMobileDateAction(null); } }}><header className="ongoing-topbar"><button className="desktop-sidebar-toggle" onClick={() => setSidebarCollapsed(value => !value)} aria-label={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"} title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}>{sidebarCollapsed ? <PanelLeftOpen size={18} /> : <PanelLeftClose size={18} />}</button><button className="mobile-menu" onClick={() => setShowSidebar(value => !value)} aria-label={showSidebar ? "Close navigation" : "Open navigation"} title={showSidebar ? "Close navigation" : "Open navigation"}>{showSidebar ? <X size={19} /> : <Menu size={19} />}</button><span className="toast-line">{toast}</span><div className="top-actions"><button onClick={goToday}><CalendarDays size={15} /> Today</button><button onClick={() => openComposer("task")}><ListTodo size={15} /> Add task</button><button onClick={() => openComposer("event")}><CalendarPlus size={15} /> Add event</button><button className="accent" onClick={() => openComposer("block")}><Plus size={16} /> Add block</button></div></header>
      ...
      {section === "accounts" && readiness.data && !readiness.data.googleOAuthReady ? <aside className="tools-note google-setup-note" role="status"><ShieldCheck size={17} /><span>{readiness.data.message}</span></aside> : null}
    </main>
    {showComposer ? <aside className="composer-reminder-dock" aria-label="Reminder timing"><Bell size={15} /><label><span>Reminder</span><select value={draftReminderLeadMinutes === "" ? "none" : String(draftReminderLeadMinutes)} onChange={event => setDraftReminderLeadMinutes(event.target.value === "none" ? "" : Number(event.target.value))}><option value="none">No off-app reminder</option><option value="0">At start time</option><option value="10">10 minutes before</option><option value="30">30 minutes before</option><option value="60">1 hour before</option><option value="1440">1 day before</option></select></label><small>Private reminder timing for this item.</small></aside> : null}
    {isMobileViewport ? <MobilePlannerNav active={mobileSurface} onNavigate={openMobileSurface} onCapture={() => setShowMobileQuickCapture(true)} /> : null}
    ...
    {showMobileQuickCapture ? <div className="mobile-quick-capture-backdrop" onPointerDown={() => setShowMobileQuickCapture(false)}><section className="mobile-quick-capture" role="dialog" aria-modal="true" aria-label="Add to MY PLAN" onPointerDown={event => event.stopPropagation()}><div className="sheet-handle" /><p className="kicker"><CirclePlus size={14} /> Quick capture</p><h2>Add what matters now.</h2><p>Start with a small next action, a protected event, or focused time. Reminder timing is available inside each item.</p><div className="quick-capture-actions"><button onClick={() => { setShowMobileQuickCapture(false); openComposer("task"); }}><ListTodo size={18} /><span><strong>Task</strong><small>Capture a next action</small></span></button><button onClick={() => { setShowMobileQuickCapture(false); openComposer("event"); }}><CalendarPlus size={18} /><span><strong>Event</strong><small>Reserve time for it</small></span></button><button onClick={() => { setShowMobileQuickCapture(false); openComposer("block"); }}><Clock3 size={18} /><span><strong>Focus block</strong><small>Make time to concentrate</small></span></button></div></section></div> : null}
