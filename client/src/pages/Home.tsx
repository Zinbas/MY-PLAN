...
export default function Home() {
  ...
  return <div className="ongoing-shell">
    <aside className={`ongoing-sidebar ${showSidebar ? "is-open" : ""}`}><div className="brand-line"><div className="tab-mark"><i /><i /><i /></div><div><span>Academic calendar</span><strong>MY PLAN</strong></div></div><div className="side-nav"><button className={section === "calendar" ? "active" : ""} onClick={() => setSection("calendar")}><CalendarDays size={17} /> Calendar</button><button className={section === "accounts" ? "active" : ""} onClick={() => setSection("accounts")}><CirclePlus size={17} /> Accounts</button><button className={section === "sync" ? "active" : ""} onClick={() => setSection("sync")}><RefreshCw size={17} /> Sync center</button><button className={section === "spark" ? "active" : ""} onClick={() => setSection("spark")}><Sparkles size={17} /> Gemini Spark</button></div><div className="side-footer"><ShieldCheck size={15} /><p>Google activation is ready when your app-owned OAuth credentials are added.</p></div></aside>
    <main className="ongoing-main"><header className="ongoing-topbar"><button className="mobile-menu" onClick={() => setShowSidebar(value => !value)} aria-label="Toggle navigation"><Menu size={19} /></button><span className="toast-line">{toast}</span><div className="top-actions"><button onClick={goToday}>Today</button><button className="accent" onClick={() => { setShowComposer(true); setSection("calendar"); }}><Plus size={16} /> Add block</button></div></header>
