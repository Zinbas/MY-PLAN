...
export default function Home() {
  ...
  const eventBlocks = useMemo<PlannerBlock[]>(() => [...personalEvents.map(event => ({ id: event.id, title: event.title, startAt: event.startAt, endAt: event.endAt, source: "event" as const, priority: event.priority, course: event.course, notes: event.notes, hasTime: event.hasTime })), ...visibleSparkEvents.map(event => ({ id: `spark-${event.id}`, title: event.title, startAt: event.startAt, endAt: event.endAt, source: "event" as const, priority: "normal" as Priority, course: "Gemini Spark", notes: event.description || "Created through your connected Spark app." }))], [personalEvents, visibleSparkEvents]);
