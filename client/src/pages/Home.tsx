...
type CalendarTaskStatusFilter = "all" | "open" | "in-progress" | "completed";
...
export default function Home() {
  ...
  const visibleEvents = useMemo(() => allCalendarBlocks.filter(event => {
    ...
    const matchesTaskStatus = calendarTaskStatusFilter === "all" || taskState === calendarTaskStatusFilter;
  ...
  }).sort((a, b) => a.startAt.getTime() - b.startAt.getTime()), [activeEnd, activeStart, allCalendarBlocks, calendarCourseFilter, calendarPriorityFilter, calendarTaskStatuses, calendarTaskStatusFilter, filter, itemTypeFilter, routineFilter, scheduleHealthFilter, search]);
  ...
  const saveComposer = () => {
    ...
    if (composerKind === "block") {
      ...
      setPlannerBlocks(current => editingId ? current.map(block => block.id === editingId ? { ...next, completed: block.completed, excludedDates: block.excludedDates } : block) : [...current, next]);
