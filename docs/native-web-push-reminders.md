...
| Local-plan enrollment | Starts only when the user chooses **Sync upcoming items** after enabling a device. It includes only title, time, type, and MY PLAN destination for eligible upcoming items. | It never uploads notes, courses, checklists, attachments, or other local workspace details; users can remove these copies without changing their browser plan. |
...
| Local-plan enrollment | A separate, clearly labeled **Sync upcoming items** action may include eligible items from the next 120 days. It is not automatic; users refresh it after editing local items. |
...
Device consent does not by itself copy local planning data to the service. The user must separately choose **Sync upcoming items**. The server rejects enrollment until an enabled current device subscription exists, omits items whose configured reminder time has already passed, and cancels the previous delivery when a user refreshes a changed or removed item. No test notification or reminder is created merely by enabling this setting.
